...
while :; do
	...
	if [ "$mode" = "child" ]; then
		...
		for i in "$(python start-servers.py -d | grep pid)"; do
		...
		while :; do
			if [ -n "$(ls -R $commondb | grep identity)" ]; then
				...
				if [ "$saved_file" = "$id_file" ]; then
					...
					pause "step3($mode): copy response file to the child directory"
					cp -f $file_init $commondb/child/
					info " parent setting done"
					saved_file=$id_file
					break;
				fi
			else
...
#
		...
		cp -f $RPKI_DIR/smoketest.dir/rootd.cer $RPKI_DIR/publication/root.cer
		sudo rsync --daemon
